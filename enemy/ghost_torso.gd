...
func _process(delta):
	...
	if bodycontrol.global_transform.origin.distance_to(playercam.global_transform.origin) < 5:
		handl.add_central_force(((playercam.global_transform.origin - handl.global_transform.origin).normalized())*10)
		handr.add_central_force(((playercam.global_transform.origin - handr.global_transform.origin).normalized())*10)
		# print('addhandforce')
